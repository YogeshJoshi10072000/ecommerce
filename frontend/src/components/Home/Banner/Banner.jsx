...
const Banner = () => {
  ...
  return (
    <>
      <section className="h-900 sm:h-100 w-full rounded-sm shadow relative overflow-hidden">
        <Slider {...settings}>
          {banners.map((el, i) => (
            <img draggable="false" className="h-80 sm:h-100 w-full object-cover" src={el} alt="banner" key={i} />
          ))}
        </Slider>
      </section>
    </>
